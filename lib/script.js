module.exports = (local) => (controller) => {
  ...
  controller.hears(/(tql|太强了)/iu, 'message', (bot, message) => {
    bot.reply(message, msg(random([
      'tql!!!',
      '太强了！！！'
    ])))
  ...
  controller.hears(/(kywkl)/iu, 'message', (bot, message) => {
    bot.reply(message, msg('→ https://wukong.hlyue.com/test'))
  })

  controller.hears(/(这首|这.?歌|啥歌|什么歌|啥歌|的歌)/iu, 'message', async (bot, message) => {
    const {data} = await axios.get('https://wukong.leeleo.me/api/channel/test/song')
    if (!data) return
    const {title, artist, webUrl} = data
    bot.reply(message, msg(`Wukong#test: ${title} - ${artist}\n${webUrl}`))
  })
  */
    ...
    for (;;) {
      ...
      for (const channel of channels) {
        ...
        const text = [
          ...
          ...stats
            .filter((item) =>
              ...
              item2.textCount - item1.textCount ||
              item2.stickerCount - item1.stickerCount
            ).map((item, index) => {
              const strings = []
              if (item.isBirthday) {
                strings.push('🎂')
        ...
        if (usersWithUpcomingBirthday.length) {
          await timeout(1000)
          bot.say(msg(`🎉${usersWithUpcomingBirthday} 要过生日了！🎉`, null, channel))
