...
export async function run() {
  for (;;) {
    ...
    for (const chat of chats) {
      ...
      const statText = util.formatJoin('\n', [
        ...
        ...stats
          ...
          .map((item, index) => {
            ...
            return util.formatJoin('', strings)
          }),
      ])
      ...
      if (birthdays.length) {
        ...
        const birthdayText = util.formatJoin(' ', [
          '🎉',
          ...birthdays.map((item) => format.mention(`@${item.username}`, item.user)),
          '要过生日了！🎉',
        ])
