...
const wukong = {
  ...
  async run() {
    ...
    while (this.state.serving) {
      ...
      const tracks = this.state.tracks[user]
      ...
      try {
        ...
        await bot.telegram.sendMessage(config.wukong.chat, `❌播放失败 [链接](${link})\n${error}`, {
          parse_mode: 'MarkdownV2',
        })
      }
    }
...
export async function setup() {
  bot
    ...
    .hears(/(?=.*(wukong|wk|悟空))(?=.*(开始|播放|加入|我也|来|start|play|join))/iu, async (context, next) => {
      const user = context.from.id
