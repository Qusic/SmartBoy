...
const wukong = {
  ...
  async load(user) {
    return store.data.wukong[user] || {netease: {}}
  },
  async save(user, data) {
    store.data.wukong[user] = data
  ...
  async tracks(playlist, cookie) {
    ...
    return response.body.playlist.tracks
      .filter((track, index) => response.body.privileges[index].st == 0)
      .map((track) => ({
        id: track.id,
        name: `${track.ar.map((artist) => artist.name).join('/')} - ${track.name}`,
      }))
  },
  ...
  async notify(user, track, link) {
    if (user && track && link) {
      const text = util.formatJoin('', [
        format.fmt`🎵[${user}] ${format.link(track, link)}\n🎧`,
        util.formatJoin(' ', [
          format.link('手机听', `${config.url}/wukong/${wukong.constant.stream}`),
          format.link('电脑听', `${config.url}/wukong/`),
        ]),
      ])
      const extra = {disable_notification: true}
      if (this.state.message) {
        await bot.telegram.editMessageText(config.chat, this.state.message, null, text, extra)
      } else {
        const message = await bot.telegram.sendMessage(config.chat, text, extra)
        this.state.message = message.message_id
        await bot.telegram.pinChatMessage(config.chat, this.state.message, extra)
      }
    } else {
      if (this.state.message) {
        await bot.telegram.unpinChatMessage(config.chat, this.state.message)
  ...
  async run() {
    ...
    while (this.state.serving) {
      ...
      try {
        ...
        await bot.telegram.sendMessage(config.chat, `❌来自${name}的歌曲播放失败\n${link}\n${error}`, {
    ...
    await this.notify()
    await this.reset()
    await bot.telegram.sendMessage(config.chat, '没人在听了，今天的悟空到此结束。谢谢大家～～')
    this.state.users = []
    this.state.names = {}
    this.state.cookies = {}
    this.state.tracks = {}
        ...
        if (data.netease.cookie && data.netease.playlist) {
          try {
            ...
            if (tracks && tracks.length) {
              const name = util.displayName(context.from)
              wukong.state.users.push(user)
              wukong.state.names[user] = name
              wukong.state.cookies[user] = data.netease.cookie
              wukong.state.tracks[user] = util.shuffle(tracks)
              await context.sendMessage(`欢迎${name}带着${tracks.length}首歌进入了悟空！！！`)
              wukong.run()
            } else {
              await context.reply('你的歌单列表好像是空的。。')
            }
          } catch (error) {
            await context.reply(`获取你的歌单列表失败了。。${error}`)
          }
        } else {
          await context.reply('你还没有设置Cookie和歌单！发送悟空关键词加上代码块或歌单链接来设置。')
        }
      })
    })
        ...
        if (data.netease.cookie && data.netease.playlist) {
          try {
            ...
            if (tracks && tracks.length) {
              wukong.state.cookies[user] = data.netease.cookie
              wukong.state.tracks[user] = util.shuffle(tracks)
              await context.reply(`歌单刷新成功，现在你的列表里有${tracks.length}首歌！`)
            } else {
              await context.reply('你的歌单列表好像是空的。。')
            }
          } catch (error) {
