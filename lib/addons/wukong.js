...
const wukong = {
  ...
  async notify(name, link) {
    if (name && link) {
      const text = [
        `🎵正在播放来自${name}的[歌曲](${link})`,
...
export async function setup() {
  bot
    ...
    .hears(/(?=.*(wukong|wk|悟空))(?=.*(退出|停止|走了|溜了|不听了|leave|stop|bye))/iu, async (context, next) => {
      const user = context.from.id
      await wukong.command(
        context.chat.id,
        user,
    ...
    .hears(/(?=.*(wukong|wk|悟空))(?=.*(换歌|切歌|下一首|下一曲))/iu, async (context, next) => {
      await wukong.command(
        context.chat.id,
        context.from.id,
        true,
