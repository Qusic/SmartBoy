...
for (const method of [
  ...
  'hget', 'hmget', 'hgetall', 'hset', 'hincrby',
  'sadd', 'smembers'
...
const dayString = (day) => [
  day[0].toString().slice(-2).padStart(2, '0'),
  day[1].toString().slice(-2).padStart(2, '0')
].join('')
...
bot.mention((value, context) => value == `@${context.me}`, (context) => {
...
bot.use(new Telegraf.Stage([(() => {
  const scene = new Telegraf.BaseScene('birthday')
  scene.enter((context) => {
    const user = context.from.id
    const text = context.message.text
    let day = null, match = null
    if ((match = text.match(/(\d{1,2})[.-月](\d{1,2})/)) &&
